...
class FeatureExtractor(object):
    ...
    def get_melspectrogram(self, X):
        def _get_melspectrogram(x):
            ...
            # mel = librosa.feature.melspectrogram(y=x, sr=self.sample_rate, n_fft=1024, hop_length=401,win_length=400,n_mels=40)[np.newaxis, :]  # 10.10 used septr version
...
def segment(wavfile, 
            ...
            overlap = 1,
            padding = None):
    if isinstance(wavfile, str):  # Check if wavfile is str
        wav_data, _ = librosa.load(wavfile, sr=sample_rate)
    elif isinstance(wavfile, np.ndarray):
        wav_data = wavfile
    else:
        raise TypeError(f'Type {type(wavfile)} is not supported.')
    X= []
...
def process(wavfiles, 
    ...
    # Split datatset
    n = len(wavfiles)
    train_indices = list(np.random.choice(range(n), int(n * split_rate), replace=False))  # Randomly select split_rate fraction of indices
    valid_indices = list(set(range(n)) - set(train_indices))
    train_files = [(wavfiles[i], labels[i]) for i in train_indices]  # Extract dataset and labels
    valid_files = [(wavfiles[i], labels[i]) for i in valid_indices]

    get_label = lambda x: x[1]
    train_info = json.dumps(Counter(map(get_label, train_files)))  # '{"0": 648, "1": 438, "2": 216, "3": 392}' Count training samples
    test_info = json.dumps(Counter(map(get_label, valid_files)))  # '{"0": 186, "3": 100, "1": 97, "2": 41}' Count validation samples
    info = {'train': train_info, 'test': test_info}  # Dictionary of train and test sample counts
    ...
    if aug == 'upsampling':
        ...
        for l, w in label_wav.items():
            nw = len(w)
            indices = list(np.random.choice(range(nw), maxval - nw, replace=True))  # Randomly select (maxval - nw) samples from 0 to nw
            for i in indices:
...
def process_IEMOCAP(datasets_path,
        ...
        myaug=None,
        **kwargs):
    if not os.path.exists(datadir): os.makedirs(datadir, exist_ok=True)
    num_label = {}
    if featuresFileName is None:
        featuresFileName = f'{datadir}/features_{features_to_use}_{impro_or_script}.pkl'
    
    if os.path.exists(datasets_path):
        wavdirname = datasets_path + '/*/sentences/wav/*/S*.wav'#'/home/pwy/LYX/dataset/iemocap/*/sentences/wav/*/S*.wav'
        allfiles = glob.glob(wavdirname)
    else:
        raise FileNotFoundError(f'{datasets_path} not existed.')
    ...
    for wavfile in allfiles:
        if len(os.path.basename(wavfile).split('-'))<5:
            ...
            continue
        label = str(os.path.basename(wavfile).split('-')[2])  # Check label identifier
        if label not in LABEL_DICT: continue
        if impro_or_script != 'all' and (impro_or_script not in wavfile): continue
        i=i+1
        wav_data, _ = librosa.load(wavfile, sr = sample_rate)
        seg_wav_len = segment_length * sample_rate  # Target segment length in samples
        wav_len = len(wav_data)
        if seg_wav_len > wav_len:
            if padding:
                n = math.ceil(seg_wav_len/wav_len)  # Round up to duplicate audio to fill segment
                wav_data = np.hstack(n*[wav_data])
            else:
                continue
        ...
        labels.append(label)
        num_label[label] = num_label.get(label, 0) + 1  # Count samples per category
