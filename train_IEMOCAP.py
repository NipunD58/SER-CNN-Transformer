...
def config(args):
    ...
    elif args.gpu is not None and args.gpu > -1 and torch.cuda.is_available():
...
def mixup_data(x, y, alpha=0):
    ...
    if torch.cuda.is_available():
        index = torch.randperm(batch_size).cuda()  # Random permutation of indices 0 to batch_size-1
    else:
...
def get_loss(model, criterion, x, y, alpha):
    if alpha > 0:
        ...
        if torch.cuda.is_available():
            ...
            ya = ya.cuda()
            yb = yb.cuda()
        ya = ya.squeeze(1)  # Remove dimension 1: (32, 1) -> (32,)
        yb = yb.squeeze(1)

        out = model(mix_x.unsqueeze(1))  # Add channel dimension
...
def train(kws):
    ...
    # Print model architecture and parameter count (no hooks - compatible with LSTM)
    print(model)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f'Total Parameters: {trainable_params / 1_000_000:.3f}M ({total_params} total, {trainable_params} trainable)')
    ...
    for i in range(kws['epochs']):
        ...
        for val in val_dict:
            ...
            if (x.size(0) == 1):
                x = torch.cat((x, x), 0)
            out = model(x.unsqueeze(1))  # Input is segments*26*57, add channel dimension
            pred = out.mean(dim=0)  # Average across segments
            pred = torch.max(pred, 0)[1].cpu().numpy()
            y_pred.append(int(pred))
        floss.write('\n')

        report = classification_report(y_true, y_pred, digits=6, target_names=target_names)  # digits: precision of floating point numbers
        report_dict = classification_report(y_true, y_pred, digits=6, target_names=target_names, output_dict=True)
        matrix = metrics.confusion_matrix(y_true, y_pred)  # Confusion matrix
...
if __name__ == '__main__':
    ...
    if not os.path.exists(kws["datadir"]): os.makedirs(kws["datadir"], exist_ok=True)
    ...
    if kws['loadFeatures'] and os.path.exists(kws['featuresFileName']):
        ...
        # Use HuggingFace dataset loader (no local dataset needed)
        from preprocessing.IEMOCAP_loader import load_and_process_hf_iemocap

        print('Local features not found. Downloading from HuggingFace...')
        train_X, train_y, val_dict, info = load_and_process_hf_iemocap(
            datadir=kws['datadir'],
            features_to_use=kws['features_to_use'],
            sample_rate=kws['sample_rate'],
            nmfcc=kws['nmfcc'],
            segment_length=kws['segment_length'],
            train_overlap=kws['train_overlap'],
            test_overlap=kws['test_overlap'],
            split_rate=kws['split_rate'],
            save_features=kws['toSaveFeatures'],
            output_filename=kws['featuresFileName'],
        )
