...
class mbBlock(nn.Module):
    def __init__(self, kernel_size, in_channel, expand_size, out_channel, nolinear, semodule, stride):
        ...
        # 3x3 (or 5x5) depthwise separable convolution
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, relative_pos):
        ...
        x = x.flatten(2).permute(0, 2, 1)  # Convert back to (B, H*W, C)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(80, 251), patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        # img_size = to_2tuple(img_size)  # Convert input to tuple, e.g. 224 -> (224, 224)
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # Merge H, W and transpose C and H*W -> (B, H*W, C)
        x = self.norm(x)
